...
from erpnext_germany.utils.ledger import LedgerScope, get_cost_centers, get_totals
from erpnext_germany.utils.vouchers import attachments as voucher_attachments
from erpnext_germany.utils.vouchers import contra_accounts, document_fields, document_number
...
def get_entries(scope: LedgerScope) -> list[frappe._dict]:
	...
	Cut with the same ``LedgerScope`` as the totals, so the sheet and the
	figure it was opened from cannot drift apart.
	"""
	gl_entry = frappe.qb.DocType("GL Entry")
	debit = gl_entry.debit_in_account_currency
	credit = gl_entry.credit_in_account_currency
...
def get_account_title(account: str) -> str:
	...
	return frappe.get_cached_value("Account", account, "account_name") or account

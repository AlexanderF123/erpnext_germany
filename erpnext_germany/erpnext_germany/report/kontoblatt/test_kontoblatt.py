...
from erpnext_germany.erpnext_germany.report.kontoblatt import kontoblatt
from erpnext_germany.utils.ledger import LedgerScope, get_totals
...
class TestKontoblatt(FrappeTestCase):
	...
	def book(
		...
		mine, theirs = ("debit_in_account_currency", "credit_in_account_currency")
		if direction != "Debit":
		...
		entry = frappe.get_doc(
			{
				...
				"bill_no": bill_no,
				"cheque_no": cheque_no,
				# ERPNext refuses a reference number without a date, so the
				# booking date stands in -- Belegfeld 2 has no date of its own.
				"cheque_date": day if cheque_no else None,
				"user_remark": remark,
				"accounts": [
					{"account": self.account, mine: amount, "cost_center": cost_center},
					{"account": self.bank, theirs: amount, "cost_center": cost_center},
				],
			}
		)
		entry.insert()
	...
	def test_a_line_carries_the_document_number_it_was_booked_with(self):
		self.book(100.0, bill_no="RE 4711", cheque_no="K 22")
	...
	def test_the_turnover_of_the_sheet_is_the_turnover_of_the_evaluation(self):
		...
		self.book(30.0, direction="Credit", on=day)

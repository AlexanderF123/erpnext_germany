...
ledger -- including how a general reversal is counted, and which entries are
in scope at all -- rather than three that happen to look alike today.

Totals come from here. The Kontoblatt reads rows rather than totals and so
runs a query of its own, but it cuts that query with the same ``LedgerScope``
and reads a reversal through the same rule (``reversal.reversal_sides``), so
the sheet and the figure it was opened from cannot drift apart.
"""
...
class LedgerScope(NamedTuple):
	...
	def apply(self, query, gl_entry):
		...
		A general reversal is a different thing and stays in -- see
		``reversal.reversal_sides`` for how it is read.
		"""
...
def get_totals(scope: LedgerScope) -> dict[str, frappe._dict]:
	...
	A general reversal is read as a minus on the side of the entry it undoes,
	so a corrected mistake leaves the turnover figures where they were instead
	of inflating both sides.
	"""
	gl_entry = frappe.qb.DocType("GL Entry")
	debit, credit = net_of_reversals(gl_entry, scope.company)
